    ...
    if let host = Page.host,
       ...
       let lang = User.parse_lang {
        ...
        if(l != .EN_GB) {
            ...
            let path = Page.path ?? ""
            _ = Page.redirect(to:Link.to(url:"https://\(pre).appudo.com/doc/" + path +  param))
            return false
        }
    }
    return true
...
public func onGetCache(ev : PageEvent) -> PageCacheResult {
    ...
    let cdir = Dir.tmpl_www
    
    if(Page.host == "appudo.com") {
        Page.redirect(to:Link.to(url:"https://www.appudo.com"))
        return .NOTCACHED
    }
    ...
    if let f = <!dir.open(t, .O_RDONLY) {
        if(p == nil || <!cdir.access(p!, mode:.R_OK)) {
            return .CACHED(f)
        }
    }
    Page.error = .NotFound
